...
class GroupNewFeedRepository {
  HttpService _httpService;

  GroupNewFeedRepository() {
    _httpService = HttpServiceImpl();
    _httpService.init();
  }
  Future<BaseBean> addGroupFeed(Map<String, dynamic> param) async {
    try {
      var response =
          await _httpService.postRequest('group/post_group_feed.php', param);
  ...
  Future<BaseBean> postImage(Map<String, dynamic> param) async {
    var request = http.MultipartRequest(
        "POST", Uri.parse("https://eku-abo.com/api/group/post_group_feed.php"));
    request.fields['user_id'] = param['user_id'];
    request.fields['group_id'] = param['group_id'];
    ...
    var pic = await http.MultipartFile.fromPath("upload", param['upload']);
